...
export const sendMessage = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (attachmentFile) {
    const fileUrl = attachmentFile.path.startsWith("http")
      ? attachmentFile.path
      : `${req.protocol}://${req.get("host")}/uploads/${attachmentFile.filename}`;

    if (attachmentFile.mimetype.startsWith("image/")) {
      messageData.image = {
        url: fileUrl,
        publicId: attachmentFile.filename,
      };
  ...
  const latestMessageText = attachmentFile
